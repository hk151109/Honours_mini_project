...
class WildfireTrainer:
    ...
    def __init__(self, config):
        ...
        # Define models - Only top 3 for fine-tuning
        self.models = {
        ...
        # Only fine-tune top 3 performing models from previous results
        self.models_to_finetune = {"ResNet50V2", "VGG16", "VGG19"}
    ...
    def build_model(self, BaseModel, input_shape, fine_tune=False):
        """Build transfer learning model.
        
        Args:
            BaseModel: Model class or string name
            input_shape: Input tensor shape
            fine_tune: If True, unfreeze top layers for fine-tuning
        """
        ...
        if isinstance(BaseModel, str):
            ...
            if fine_tune:
                ...
                freeze_until = max(0, n_layers - self.config['unfreeze_layers'])
                for layer in base.layers[:freeze_until]:
                    layer.trainable = False
                print(f"  Fine-tuning: Unfreezing top {self.config['unfreeze_layers']} layers (layers {freeze_until}-{n_layers})")
            else:
                # Freeze all layers for initial training
                base.trainable = False
                print(f"  Initial training: All base layers frozen")
    ...
    def train_models(self):
        """Train all models (fine-tuning only)."""
        print("\n" + "="*60)
        print("Fine-tuning pre-trained models (loading from saved models)...")
        print("="*60)
        
        for name, BaseModel in tqdm(self.models.items(), desc="Fine-tuning models"):
            print(f"\n{'='*60}")
            ...
            try:
                # Try to load pre-trained model from previous run
                pretrained_path = os.path.join(self.config['models_dir'], f'{name}_best.keras')
                ...
                if os.path.exists(pretrained_path):
                    ...
                    # Create a dummy history for stage 1 (already completed)
                    history = type('History', (), {'history': {
                        'accuracy': [], 'val_accuracy': [], 
                        'loss': [], 'val_loss': []
                    }})()
                else:
                    print(f"\n⚠ Pre-trained model not found at: {pretrained_path}")
                    print("--- Stage 1: Initial Training (Frozen Base) ---")
                    model = self.build_model(BaseModel, input_shape=input_shape, fine_tune=False)
                    ...
                    callbacks = [
                        ...
                        ModelCheckpoint(
                            filepath=os.path.join(self.config['models_dir'], f'{name}_stage1.keras'),
                ...
                # Stage 2: Fine-tuning (always enabled for top 3 models)
                print(f"\n--- Stage 2: Fine-Tuning (Unfrozen Top Layers) ---")
                
                # Get the base model from the first layer
                ...
                print(f"  Unfreezing top {self.config['unfreeze_layers']} layers (layers {freeze_until}-{n_layers})")
                ...
                ft_callbacks = [
                    ...
                    ModelCheckpoint(
                        filepath=os.path.join(self.config['models_dir'], f'{name}_finetuned.keras'),
                        monitor='val_accuracy',
                        save_best_only=True,
                        verbose=0
                    )
                ...
                # Combine histories (if stage 1 was done)
                ...
                print(f"✓ Fine-tuning completed for {name}")
                
                # Save final fine-tuned model
                model.save(os.path.join(self.config['models_dir'], f'{name}_finetuned_final.keras'))
...
CONFIG = {
    ...
    'fine_tune': True,          # Enable fine-tuning after initial training
